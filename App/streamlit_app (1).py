...
# CONFIGURATION
# ═══════════════════════════════════════════════
BRANDING = "CineMetrics AI \u2022 Built by [Dev Manghani] \u2022 Microsoft Elevate 2026"

st.set_page_config(
    page_title="CineMetrics AI",
    page_icon="\U0001F3A5",
    layout="wide",
...
# ═══════════════════════════════════════════════
# CINEMETRICS AI — DESIGN SYSTEM
# ═══════════════════════════════════════════════
st.markdown("""
<style>
    /* ── Google Fonts ── */
    @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600;700&family=Outfit:wght@400;600;700;800;900&display=swap');
    ...
    .stApp {
        background: linear-gradient(135deg, #060d0b 0%, #0a1a16 25%, #0d2420 50%, #0a1a16 75%, #060d0b 100%);
    ...
    h1 {
        ...
        background: linear-gradient(135deg, #0d9488, #0f766e, #f59e0b, #f97316, #fbbf24, #0d9488);
    ...
    .stTabs [aria-selected="true"] {
        background: linear-gradient(135deg, rgba(13,148,136,0.2), rgba(118,75,162,0.2)) !important;
        color: #ffffff !important;
        box-shadow: 0 0 20px rgba(13,148,136,0.3), inset 0 1px 0 rgba(255,255,255,0.1);
    ...
    .metric-card::before {
        ...
        height: 3px;
        background: linear-gradient(90deg, #0d9488, #0f766e, #f59e0b);
    ...
    .metric-card:hover {
        ...
        box-shadow: 0 16px 48px rgba(13, 148, 136, 0.2), 0 0 30px rgba(13, 148, 136, 0.1);
    }
    .metric-icon  { font-size: 2.2rem; margin-bottom: 8px; display: block; }
    .metric-label {
        font-family: 'DM Sans', sans-serif; font-size: 0.8rem; color: #7aada8;
    ...
    .cinema-score {
        display: flex; align-items: center; justify-content: center;
        padding: 0 16px; font-family: 'Outfit', sans-serif;
    ...
    .stButton > button:hover {
        ...
        box-shadow: 0 8px 25px rgba(13, 148, 136, 0.5), 0 0 30px rgba(13, 148, 136, 0.2) !important;
    }
    ...
    .stTextArea textarea:focus {
        ...
        box-shadow: 0 0 0 3px rgba(13, 148, 136, 0.25), 0 0 20px rgba(13, 148, 136, 0.1) !important;
    ...
    .context-badge.fire     { background: rgba(249,115,22,0.15);  color: #f97316; border: 1px solid rgba(249,115,22,0.3); }
    .context-badge.warm     { background: rgba(251,191,36,0.15); color: #fbbf24; border: 1px solid rgba(251,191,36,0.3); }
    .context-badge.moderate { background: rgba(13,148,136,0.15); color: #0d9488; border: 1px solid rgba(13,148,136,0.3); }
    .context-badge.cool     { background: rgba(56,189,248,0.15); color: #38bdf8; border: 1px solid rgba(56,189,248,0.3); }
    ...
    .subtitle-typewriter {
        text-align: center; font-family: 'DM Sans', sans-serif;
        font-size: 1.08rem; color: #e2f5f3; margin-top: -10px;
        margin-bottom: 30px; font-weight: 400; letter-spacing: 0.5px;
    ...
    .hero-container {
        ...
        margin-bottom: 35px;
        border-radius: 24px;
        background: linear-gradient(135deg, rgba(13,148,136,0.08) 0%, rgba(245,158,11,0.08) 50%, rgba(249,115,22,0.06) 100%);
        border: 1px solid rgba(255,255,255,0.06);
        backdrop-filter: blur(20px);
        overflow: hidden;
    }
    .hero-container::before {
        ...
        background: linear-gradient(90deg, #0d9488, #0f766e, #f59e0b, #f97316, #fbbf24);
        background-size: 300% 100%;
        animation: gradientShift 6s ease infinite;
    }
    .hero-tagline {
        font-family: 'DM Sans', sans-serif;
        font-size: 1rem; color: #6b9e9a;
    ...
    .hero-stat-label {
        font-family: 'DM Sans', sans-serif;
        font-size: 0.72rem; color: #6b9e9a;
    ...
    .model-card-title {
        ...
        color: #f0fdf9; margin-bottom: 4px;
    }
    .model-card-sub {
        font-family: 'DM Sans', sans-serif;
        font-size: 0.82rem; color: #6b9e9a;
    ...
    .model-metric-name {
        font-family: 'DM Sans', sans-serif;
        font-size: 0.85rem; color: #7aada8;
    }
    .model-metric-value {
        font-family: 'Outfit', sans-serif;
    ...
    .footer-links a {
        color: #6b9e9a; text-decoration: none;
    ...
    .stSpinner {
        background: rgba(8, 12, 16, 0.6) !important;
...
def style_chart(fig, ax):
    ...
    ax.tick_params(colors='#7aada8', labelsize=9)
    ax.xaxis.label.set_color('#7aada8')
    ax.yaxis.label.set_color('#7aada8')
    ax.title.set_color('#e2f5f3')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.spines['bottom'].set_color('#1a3330')
    ax.spines['left'].set_color('#1a3330')
    for spine in ax.spines.values():
        spine.set_linewidth(0.5)
    ax.grid(axis='y', color='#0f2420', linewidth=0.5, alpha=0.5)
...
with st.sidebar:
    ...
    <div style='text-align:center; padding: 25px 10px 15px 10px;'>
        ...
        <p style='color: #6b9e9a; font-size: 0.85rem; font-family: 'DM Sans', sans-serif;'>
            Film Intelligence & Analytics Platform
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <p style='color:#7aada8; font-size:0.85rem; line-height:1.6;'>
    This AI-powered dashboard combines <b style='color:#5eead4;'>Machine Learning</b>,
    <b style='color:#5eead4;'>Natural Language Processing</b>, and
    <b style='color:#5eead4;'>Data Analytics</b> to deliver intelligent insights
    ...
    st.markdown("""
    <p style='color:#7aada8; font-size:0.82rem; line-height:1.8;'>
    \U0001F4CA <b style='color:#e2f5f3;'>Frontend:</b> Streamlit<br>
    \U0001F916 <b style='color:#e2f5f3;'>ML Engine:</b> scikit-learn<br>
    \U0001F4AC <b style='color:#e2f5f3;'>NLP:</b> NLTK + TF-IDF<br>
    \U0001F4C1 <b style='color:#e2f5f3;'>Data:</b> TMDB 5000, MovieLens<br>
    \U0001F4C8 <b style='color:#e2f5f3;'>Visualization:</b> Matplotlib, Seaborn
    </p>
    """, unsafe_allow_html=True)
    ...
    <div style='text-align: center; padding: 15px 10px;
        background: rgba(13,148,136,0.08); border-radius: 12px;
        border: 1px solid rgba(13,148,136,0.15);'>
        <p style='color:#5eead4; font-family:'Outfit',sans-serif;
           font-weight:700; font-size:0.9rem; margin-bottom:4px;'>
           Made with \u2764\uFE0F by Saumil Jani
        </p>
        <p style='color:#0d9488; font-family:'DM Sans',sans-serif;
...
# MAIN TITLE
# ═══════════════════════════════════════════════
st.title("\U0001F3AC CineMetrics AI")
st.markdown("<div class='subtitle-wrapper'><span class='subtitle-typewriter'>Data-Driven Cinema &bull; Smart Discovery &bull; Predictive Analytics for the Modern Film Era</span></div>", unsafe_allow_html=True)
...
<div class='hero-container fade-in'>
    <p class='hero-tagline'>
        ...
        <strong style='color:#5eead4;'>machine learning</strong>,
        <strong style='color:#5eead4;'>natural language processing</strong>, and
        <strong style='color:#5eead4;'>predictive analytics</strong>
...
# ═══════════════════════════════════════════════
with tab_eda:
    st.header("Cinema Market Intelligence")
    st.markdown("<p class='section-desc'>Deep-dive into viewing trends, genre landscapes, and rating patterns across our extensive film database.</p>", unsafe_allow_html=True)
    ...
    if movies_df is not None and ratings_df is not None:
        ...
        with col_fig1:
            ...
            palette = ['#0d4f4a','#0d6b64','#0d8c84','#0dada0','#14b8a6','#2dd4bf','#5eead4','#81e8da','#a7f3d0','#d1fae5']
        ...
        with col_fig2:
            ...
            ax2.fill_between(year_counts.index, year_counts.values, alpha=0.15, color='#0d9488')
            ax2.plot(year_counts.index, year_counts.values, color='#0d9488', linewidth=2.5)
            ax2.scatter(year_counts.index, year_counts.values, color='#f59e0b', s=15, zorder=5)
        ...
        if 'genres' in movies_df.columns:
            ...
            donut_colors = [
                '#0d9488', '#0f766e', '#f59e0b', '#f97316', '#fbbf24',
                '#4ade80', '#38bdf8', '#a78bfa', '#f472b6', '#fb923c',
                '#34d399', '#818cf8'
            ]
            wedges, texts, autotexts = ax4.pie(
                ...
                startangle=140,
                pctdistance=0.82,
                wedgeprops=dict(width=0.45, edgecolor='#080c10', linewidth=2)
            )
            for text in texts:
                text.set_color('#e2f5f3')
                text.set_fontsize(9)
                text.set_fontfamily('sans-serif')
            for autotext in autotexts:
                autotext.set_color('#f0fdf9')
                autotext.set_fontsize(7.5)
                autotext.set_fontweight('bold')

            # Center circle for donut effect
            centre_circle = plt.Circle((0, 0), 0.55, fc='#080c10')
            ax4.add_artist(centre_circle)
            ax4.text(0, 0.06, f'{len(genre_counts)}', ha='center', va='center',
                     fontsize=28, fontweight='bold', color='#2dd4bf', fontfamily='sans-serif')
            ax4.text(0, -0.1, 'Genres', ha='center', va='center',
                     fontsize=10, color='#6b9e9a', fontfamily='sans-serif')
...
# ═══════════════════════════════════════════════
with tab_rec:
    st.header("AI Film Discovery Engine")
    st.markdown("<p class='section-desc'>Pick a film you love and our engine will surface handpicked matches using genre-based semantic similarity scoring.</p>", unsafe_allow_html=True)

    if rec_model is not None:
        movies_ref = rec_model["movies"]
        genre_matrix = rec_model["genre_matrix"]

        selected_movie = st.selectbox("Search for a film you enjoy:", movies_ref["title"].values, key="rec_select")
...
with tab_sent:
    ...
    st.markdown("<p class='section-desc'>Drop any film review below and our NLP pipeline will classify the sentiment instantly using TF-IDF and Logistic Regression.</p>", unsafe_allow_html=True)

    user_review = st.text_area(
        "Enter a film review or comment:",
        height=160,
    ...
    if st.session_state.sent_result is not None:
        ...
        <div class='result-panel {panel_class} fade-in'>
            ...
            <p style='color:#7aada8; font-size: 0.95rem; margin-bottom: 15px;'>{desc_text}</p>
            <div style='display:flex; align-items:center; justify-content:center; gap:12px;'>
                <span style='color:#e2f5f3; font-family:'DM Sans',sans-serif; font-weight:600; font-size:1rem;'>Confidence</span>
                <span style='color:{glow_color}; font-family:'Outfit',sans-serif; font-weight:800; font-size:2rem;'>{r["confidence"]:.1f}%</span>
            </div>
...
# ═══════════════════════════════════════════════
with tab_pop:
    st.header("Box Office Popularity Predictor")
    st.markdown("<p class='section-desc'>Model how a hypothetical film might perform using real TMDB data. Tune the budget, runtime, and genre mix to generate a predicted popularity score.</p>", unsafe_allow_html=True)
        ...
        if st.button("Predict TMDB Popularity \U0001F52E", type="primary", key="pop_btn"):
            if selected_genres:
                ...
                <div class='result-panel fade-in' style='border-color: {score_color}33; box-shadow: 0 8px 32px {score_color}15, 0 0 60px {score_color}08;'>
                    <p style='color:#7aada8; font-family:'DM Sans',sans-serif; font-size:0.85rem; text-transform:uppercase; letter-spacing:2px; margin-bottom:10px;'>Predicted TMDB Popularity Score</p>
                    <div style='font-family:'Outfit',sans-serif; font-weight:900; font-size:5rem; line-height:1;
                        background: linear-gradient(135deg, #0d9488, {score_color});
                        -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;'>
                        {pred_pop:.1f}
                    </div>
                    <div class='conf-bar-track' style='margin-top:20px; height:12px;'>
                        <div class='conf-bar-fill' style='width:{pct}%; background:linear-gradient(90deg, #0d9488, {score_color}); height:100%;'></div>
                    </div>
                ...
                <div style='display:flex; justify-content:center; gap:30px; margin-top:20px;'>
                    <div style='text-align:center;'>
                        <p style='color:#7aada8; font-size:0.75rem; text-transform:uppercase; letter-spacing:1px; margin-bottom:2px;'>Budget</p>
                        <p style='color:#f0fdf9; font-family:'Outfit',sans-serif; font-weight:700; font-size:1.1rem;'>{budget_str}</p>
                    </div>
                    <div style='text-align:center;'>
                        <p style='color:#7aada8; font-size:0.75rem; text-transform:uppercase; letter-spacing:1px; margin-bottom:2px;'>Runtime</p>
                        <p style='color:#f0fdf9; font-family:'Outfit',sans-serif; font-weight:700; font-size:1.1rem;'>{runtime} min</p>
                    </div>
                    <div style='text-align:center;'>
                        <p style='color:#7aada8; font-size:0.75rem; text-transform:uppercase; letter-spacing:1px; margin-bottom:2px;'>Genres</p>
                        <p style='color:#f0fdf9; font-family:'Outfit',sans-serif; font-weight:700; font-size:1.1rem;'>{', '.join(selected_genres)}</p>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            else:
...
# ═══════════════════════════════════════════════
with tab_model:
    st.header("Model Performance Lab")
    st.markdown("<p class='section-desc'>Inspect training metrics, architecture details, and evaluation results for each ML model in the CineMetrics pipeline.</p>", unsafe_allow_html=True)
    ...
    with col_m1:
        ...
        <div class='model-card fade-in' style='animation-delay: 0s;'>
            <div style='position:absolute; top:0; left:0; right:0; height:3px; background:linear-gradient(90deg, #0d9488, #0f766e); border-radius:18px 18px 0 0;'></div>
            ...
            <div class='model-metric-row'>
                ...
                <span class='model-metric-value' style='color:#5eead4;'>Cosine Sim</span>
            </div>
            <div class='model-metric-row'>
                <span class='model-metric-name'>Movies Indexed</span>
                <span class='model-metric-value' style='color:#f59e0b;'>{rec_movies_count:,}</span>
            </div>
            <div class='model-metric-row'>
                <span class='model-metric-name'>Feature Dims</span>
                <span class='model-metric-value' style='color:#fbbf24;'>{rec_features}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_m2:
        ...
        <div class='model-card fade-in' style='animation-delay: 0.15s;'>
            ...
            <div class='model-metric-row'>
                <span class='model-metric-name'>Algorithm</span>
                <span class='model-metric-value' style='color:#5eead4;'>{sent_algo}</span>
            </div>
            <div class='model-metric-row'>
                <span class='model-metric-name'>Vectorizer</span>
                <span class='model-metric-value' style='color:#f59e0b;'>{sent_vectorizer}</span>
            </div>
            <div class='model-metric-row'>
                <span class='model-metric-name'>Vocabulary</span>
                <span class='model-metric-value' style='color:#fbbf24;'>{vocab_size:,}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_m3:
        ...
        <div class='model-card fade-in' style='animation-delay: 0.3s;'>
            <div style='position:absolute; top:0; left:0; right:0; height:3px; background:linear-gradient(90deg, #f97316, #fbbf24); border-radius:18px 18px 0 0;'></div>
            ...
            <div class='model-metric-row'>
                <span class='model-metric-name'>Algorithm</span>
                <span class='model-metric-value' style='color:#5eead4;'>Random Forest</span>
            </div>
            <div class='model-metric-row'>
                <span class='model-metric-name'>Features</span>
                <span class='model-metric-value' style='color:#f59e0b;'>{pop_features_count}</span>
            </div>
            <div class='model-metric-row'>
                <span class='model-metric-name'>R\u00b2 Score</span>
                <span class='model-metric-value' style='color:#fbbf24;'>{r2_display}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        try:
            ...
            with eval_col2:
                ...
                sns.heatmap(cm, annot=True, fmt='d', cmap='YlGnBu',
                            ...
                            linewidths=2, linecolor='#080c10',
                            annot_kws={'fontsize': 16, 'fontweight': 'bold'})
                ax_cm.set_xlabel('Predicted', fontsize=11, color='#7aada8')
                ax_cm.set_ylabel('Actual', fontsize=11, color='#7aada8')
                ax_cm.tick_params(colors='#7aada8', labelsize=10)
    ...
    <div class='model-card'>
        <div style='display:flex; flex-wrap:wrap; gap:20px; justify-content:center; text-align:center;'>
            <div style='flex:1; min-width:180px; padding:15px;'>
                ...
                <div style='color:#f0fdf9; font-family:'Outfit',sans-serif; font-weight:700; font-size:0.95rem;'>Data Layer</div>
                <div style='color:#6b9e9a; font-size:0.8rem; margin-top:6px;'>TMDB 5000 + MovieLens<br>Cleaned & Preprocessed CSVs</div>
            </div>
            <div style='color:#0d9488; font-size:1.5rem; display:flex; align-items:center;'>\u27A1</div>
            <div style='flex:1; min-width:180px; padding:15px;'>
                <div style='font-size:2rem; margin-bottom:8px;'>\U0001F9E0</div>
                <div style='color:#f0fdf9; font-family:'Outfit',sans-serif; font-weight:700; font-size:0.95rem;'>ML Engine</div>
                <div style='color:#6b9e9a; font-size:0.8rem; margin-top:6px;'>scikit-learn Pipelines<br>TF-IDF, Random Forest, LogReg</div>
            </div>
            <div style='color:#0d9488; font-size:1.5rem; display:flex; align-items:center;'>\u27A1</div>
            <div style='flex:1; min-width:180px; padding:15px;'>
                <div style='font-size:2rem; margin-bottom:8px;'>\U0001F310</div>
                <div style='color:#f0fdf9; font-family:'Outfit',sans-serif; font-weight:700; font-size:0.95rem;'>Presentation Layer</div>
                <div style='color:#6b9e9a; font-size:0.8rem; margin-top:6px;'>Streamlit Dashboard<br>Interactive UI + PDF Reports</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
padding: 40px 45px;
margin: 25px 0;
box-shadow: 0 12px 48px rgba(13,148,136,0.1), 0 0 80px rgba(13,148,136,0.05);
position: relative;
overflow: hidden;
'>
...
<h3 style='margin:0; font-family:'Outfit',sans-serif; font-weight:700; font-size:1.2rem;
background:linear-gradient(135deg,#0d9488,#f59e0b);
-webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;'>
Project Overview
</h3>
</div>
<p style='color:#e2f5f3; font-size:0.92rem; line-height:1.7; margin:0; font-family:'DM Sans',sans-serif;'>
<strong style='color:#f0fdf9;'>CineMetrics AI</strong> is an intelligent media analytics platform
that tackles the problem of <strong style='color:#f59e0b;'>content overload</strong> in today's entertainment landscape.
Using AI-powered recommendation engines, NLP-based sentiment analysis, and predictive
popularity modeling, it delivers <strong style='color:#5eead4;'>personalized, data-driven insights</strong>
...
background:rgba(13,148,136,0.08); border-radius:10px; border:1px solid rgba(13,148,136,0.12);'>
<span style='font-size:1.1rem;'>\U0001F916</span>
<span style='color:#e2f5f3; font-size:0.88rem; font-family:'DM Sans',sans-serif;'>Personalized Movie Recommendations</span>
</div>
<div style='display:flex; align-items:center; gap:8px; padding:10px 14px;
background:rgba(245,158,11,0.08); border-radius:10px; border:1px solid rgba(245,158,11,0.12);'>
<span style='font-size:1.1rem;'>\U0001F4CA</span>
<span style='color:#e2f5f3; font-size:0.88rem; font-family:'DM Sans',sans-serif;'>Interactive Data Visualization & EDA</span>
</div>
<div style='display:flex; align-items:center; gap:8px; padding:10px 14px;
...
<span style='color:#e2f5f3; font-size:0.88rem; font-family:'DM Sans',sans-serif;'>NLP Sentiment Analysis of Reviews</span>
</div>
<div style='display:flex; align-items:center; gap:8px; padding:10px 14px;
background:rgba(249,115,22,0.08); border-radius:10px; border:1px solid rgba(249,115,22,0.12);'>
<span style='font-size:1.1rem;'>\U0001F680</span>
<span style='color:#e2f5f3; font-size:0.88rem; font-family:'DM Sans',sans-serif;'>TMDB Popularity Score Prediction</span>
</div>
<div style='display:flex; align-items:center; gap:8px; padding:10px 14px;
background:rgba(251,191,36,0.08); border-radius:10px; border:1px solid rgba(251,191,36,0.12);'>
<span style='font-size:1.1rem;'>\U0001F4C4</span>
<span style='color:#e2f5f3; font-size:0.88rem; font-family:'DM Sans',sans-serif;'>PDF Report Generation & Export</span>
</div>
<div style='display:flex; align-items:center; gap:8px; padding:10px 14px;
background:rgba(56,189,248,0.08); border-radius:10px; border:1px solid rgba(56,189,248,0.12);'>
<span style='font-size:1.1rem;'>\U0001F3AF</span>
<span style='color:#e2f5f3; font-size:0.88rem; font-family:'DM Sans',sans-serif;'>Content-Based Genre Filtering</span>
</div>
</div>
</div>
...
<span style='padding:8px 18px; background:rgba(13,148,136,0.12); border:1px solid rgba(13,148,136,0.2);
border-radius:25px; color:#5eead4; font-family:'DM Sans',sans-serif; font-size:0.82rem; font-weight:600;'>
\U0001F4CA Streamlit
</span>
<span style='padding:8px 18px; background:rgba(74,222,128,0.12); border:1px solid rgba(74,222,128,0.2);
border-radius:25px; color:#4ade80; font-family:'DM Sans',sans-serif; font-size:0.82rem; font-weight:600;'>
\U0001F40D Python
</span>
<span style='padding:8px 18px; background:rgba(245,158,11,0.12); border:1px solid rgba(245,158,11,0.2);
border-radius:25px; color:#f59e0b; font-family:'DM Sans',sans-serif; font-size:0.82rem; font-weight:600;'>
\U0001F9E0 scikit-learn
</span>
<span style='padding:8px 18px; background:rgba(251,191,36,0.12); border:1px solid rgba(251,191,36,0.2);
border-radius:25px; color:#fbbf24; font-family:'DM Sans',sans-serif; font-size:0.82rem; font-weight:600;'>
\U0001F4CA Pandas & NumPy
</span>
<span style='padding:8px 18px; background:rgba(249,115,22,0.12); border:1px solid rgba(249,115,22,0.2);
border-radius:25px; color:#f97316; font-family:'DM Sans',sans-serif; font-size:0.82rem; font-weight:600;'>
\U0001F3A8 Matplotlib & Seaborn
</span>
<span style='padding:8px 18px; background:rgba(56,189,248,0.12); border:1px solid rgba(56,189,248,0.2);
border-radius:25px; color:#38bdf8; font-family:'DM Sans',sans-serif; font-size:0.82rem; font-weight:600;'>
\U0001F4DD NLTK (NLP)
</span>
</div>
</div>
...
<div style='color:#e2f5f3; font-size:0.82rem; font-weight:600; font-family:'DM Sans',sans-serif;'>Select a tab</div>
<div style='color:#6b9e9a; font-size:0.75rem; margin-top:4px;'>Choose your analysis</div>
</div>
<div style='flex:1; min-width:140px; text-align:center; padding:16px 12px;
background:rgba(15,15,35,0.6); border-radius:14px; border:1px solid rgba(255,255,255,0.06);'>
<div style='font-size:1.8rem; margin-bottom:6px;'>2\uFE0F\u20E3</div>
<div style='color:#e2f5f3; font-size:0.82rem; font-weight:600; font-family:'DM Sans',sans-serif;'>Enter input</div>
<div style='color:#6b9e9a; font-size:0.75rem; margin-top:4px;'>Movie, review, or budget</div>
</div>
<div style='flex:1; min-width:140px; text-align:center; padding:16px 12px;
background:rgba(15,15,35,0.6); border-radius:14px; border:1px solid rgba(255,255,255,0.06);'>
<div style='font-size:1.8rem; margin-bottom:6px;'>3\uFE0F\u20E3</div>
<div style='color:#e2f5f3; font-size:0.82rem; font-weight:600; font-family:'DM Sans',sans-serif;'>Click analyze</div>
<div style='color:#6b9e9a; font-size:0.75rem; margin-top:4px;'>Run the AI model</div>
</div>
<div style='flex:1; min-width:140px; text-align:center; padding:16px 12px;
background:rgba(15,15,35,0.6); border-radius:14px; border:1px solid rgba(255,255,255,0.06);'>
<div style='font-size:1.8rem; margin-bottom:6px;'>4\uFE0F\u20E3</div>
<div style='color:#e2f5f3; font-size:0.82rem; font-weight:600; font-family:'DM Sans',sans-serif;'>View results</div>
<div style='color:#6b9e9a; font-size:0.75rem; margin-top:4px;'>Explore & export PDF</div>
</div>
</div>
</div>
...
border:1px solid rgba(255,255,255,0.06);'>
<div style='color:#7aada8; font-size:0.75rem; text-transform:uppercase;
letter-spacing:1px; margin-bottom:6px; font-family:'DM Sans',sans-serif;'>Source</div>
<div style='color:#f0fdf9; font-size:0.92rem; font-weight:600; font-family:'DM Sans',sans-serif;'>
...
border:1px solid rgba(255,255,255,0.06);'>
<div style='color:#7aada8; font-size:0.75rem; text-transform:uppercase;
letter-spacing:1px; margin-bottom:6px; font-family:'DM Sans',sans-serif;'>Scale</div>
<div style='color:#f0fdf9; font-size:0.92rem; font-weight:600; font-family:'DM Sans',sans-serif;'>
...
<div style='color:#7aada8; font-size:0.75rem; text-transform:uppercase;
letter-spacing:1px; margin-bottom:6px; font-family:'DM Sans',sans-serif;'>Key Columns</div>
<div style='color:#e2f5f3; font-size:0.88rem; font-family:'DM Sans',sans-serif; line-height:1.6;'>
...
<div style='padding:18px; background:rgba(13,148,136,0.06); border-radius:14px;
border:1px solid rgba(13,148,136,0.15);'>
<div style='color:#5eead4; font-family:'Outfit',sans-serif; font-weight:700;
font-size:0.95rem; margin-bottom:8px;'>Content-Based Filtering</div>
<div style='color:#94A3B8; font-size:0.82rem; line-height:1.6; font-family:'DM Sans',sans-serif;'>
...
<div style='color:#4ade80; font-family:'Outfit',sans-serif; font-weight:700;
...
<div style='padding:18px; background:rgba(249,115,22,0.06); border-radius:14px;
border:1px solid rgba(249,115,22,0.15);'>
<div style='color:#f97316; font-family:'Outfit',sans-serif; font-weight:700;
...
<div style='display:flex; flex-wrap:wrap; gap:10px;'>
<span style='padding:8px 16px; background:rgba(13,148,136,0.08); border:1px solid rgba(13,148,136,0.15);
border-radius:10px; color:#5eead4; font-size:0.82rem; font-family:'DM Sans',sans-serif; font-weight:500;'>
\U0001F310 Real-time streaming data integration
</span>
<span style='padding:8px 16px; background:rgba(245,158,11,0.08); border:1px solid rgba(245,158,11,0.15);
border-radius:10px; color:#f59e0b; font-size:0.82rem; font-family:'DM Sans',sans-serif; font-weight:500;'>
...
border-radius:10px; color:#4ade80; font-size:0.82rem; font-family:'DM Sans',sans-serif; font-weight:500;'>
\U0001F4F1 Mobile-responsive deployment
</span>
<span style='padding:8px 16px; background:rgba(251,191,36,0.08); border:1px solid rgba(251,191,36,0.15);
border-radius:10px; color:#fbbf24; font-size:0.82rem; font-family:'DM Sans',sans-serif; font-weight:500;'>
\U0001F465 Collaborative filtering with user profiles
</span>
<span style='padding:8px 16px; background:rgba(56,189,248,0.08); border:1px solid rgba(56,189,248,0.15);
border-radius:10px; color:#38bdf8; font-size:0.82rem; font-family:'DM Sans',sans-serif; font-weight:500;'>
...
<div class='pro-footer'>
    <div class='footer-brand'>\U0001F3AC CineMetrics AI</div>
    <div class='footer-copy'>
        {BRANDING} &bull; v2.0 &bull; Powered by scikit-learn &bull; Built with Streamlit<br>
        &copy; 2026 [Dev Manghani]. All rights reserved.
    </div>
</div>
""", unsafe_allow_html=True)
